...
export default function App() {
  ...
  const [pendingBooking, setPendingBooking] = useState<{ movie: Movie; showtime?: Showtime | null } | null>(null);
  ...
  const continuePendingBooking = () => {
    ...
    const { movie, showtime } = pendingBooking;
    ...
    if (showtime) {
      ...
      setSelectedShowtime(null);
      setDetailScrollToShowtimes(true);
  ...
  const handleMovieClick = (movie: Movie, scrollToShowtimes = false) => {
    if (scrollToShowtimes && !userEmail) {
      setPendingBooking({ movie });
      setPage("auth");
      setAuthSubMode("login");
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
  ...
  const handleSelectShowtime = (showtime: Showtime, movie: Movie) => {
    if (!userEmail) {
      setPendingBooking({ movie, showtime });
      setPage("auth");
      setAuthSubMode("login");
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
  ...
  const handleConfirmSeats = (
    ...
    if (!userEmail) {
      setPendingBooking({ movie: selectedMovie!, showtime: selectedShowtime });
      setPage("auth");
      setAuthSubMode("login");
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col font-sans selection:bg-red-600 selection:text-white">
      ...
      <main className="flex-1">
        {page === "admin" && (
          <AdminPortalPage userRole={userRole} onBack={() => navigate("home")} />
        )}
