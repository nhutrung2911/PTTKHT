...
const generateDates = () => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    showDates.push(`${day}/${month}`);
    
    const localTime = new Date(d.getTime() - d.getTimezoneOffset() * 60000);
    dateValues.push(localTime.toISOString().split("T")[0]);
    
    dayNames.push(dayLabels[d.getDay()]);
  }
  return { showDates, dateValues, dayNames };
};

const { showDates: SHOW_DATES, dateValues: DATE_VALUES, dayNames: DAY_NAMES } = generateDates();
